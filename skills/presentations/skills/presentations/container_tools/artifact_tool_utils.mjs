...
function isArtifactToolPackage(packageDir) {
  return isNamedPackage(packageDir, "office-artifact-tool");
}
...
function validateArtifactToolPackage(packageDir, context) {
  ...
  if (!isArtifactToolPackage(packageDir)) {
    ...
    throw new Error(
      [
        `Expected ${context} to point to office-artifact-tool.`,
  ...
  if (compareSemver(packageJson.version, MIN_ARTIFACT_TOOL_VERSION) < 0) {
    throw new Error(
      [
        `Expected ${context} to point to office-artifact-tool ${MIN_ARTIFACT_TOOL_VERSION} or newer.`,
        `Checked ${packageDir}; found office-artifact-tool@${packageJson.version || "unknown"}.`,
      ].join("\n"),
    );
  }
...
function findArtifactToolPackage() {
  return validateArtifactToolPackage(
    runtimePackagePath("office-artifact-tool"),
    "the bundled Codex runtime office-artifact-tool package",
  );
}
...
export async function ensureArtifactToolWorkspace(workspaceDir) {
  ...
  await ensureWorkspacePackage(resolvedWorkspace, "office-artifact-tool", sourcePackage);
